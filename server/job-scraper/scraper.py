...
def process_linkedin_query(search_query: str, location: str, limit: int = None, existing_ids: set = None) -> list:
    """
    Orchestrates scraping and detail fetching for a single query,
    filtering against existing jobs BEFORE fetching details.
    ...
    logging.info("\n--- Starting Filtering Step: Checking against existing IDs ---")
    if existing_ids is None:
        try:
            job_ids_set, company_title_set = supabase_utils.get_existing_jobs_from_supabase()
        except Exception:
            job_ids_set = set()
            company_title_set = set()
    else:
        job_ids_set = existing_ids
        company_title_set = set()
    ...
    logging.info(f"Found {len(job_ids_set)} existing IDs.")
...
def process_careers_future_query(search_query: str, limit: int = None, existing_ids: set = None) -> list:
    """
    ...
    # 2. Fetch existing job identifiers
    logging.info("Phase 2: Fetching existing job identifiers...")
    if existing_ids is None:
        try:
            job_ids_set_supabase, company_title_set_supabase = supabase_utils.get_existing_jobs_from_supabase()
            logging.info(f"Phase 2: Supabase returned {len(job_ids_set_supabase)} existing IDs.")
        except Exception as e:
            logging.error(f"Failed to fetch existing jobs: {e}")
            logging.warning("Proceeding with empty set; all fetched jobs will be considered new.")
            job_ids_set_supabase = set()
            company_title_set_supabase = set()
    else:
        job_ids_set_supabase = existing_ids
        company_title_set_supabase = set()
